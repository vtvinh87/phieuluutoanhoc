...
import DailyChallengeModal from './DailyChallengeModal'; // New Import
...
import {
    ...
    MailIcon as MessageIcon, PlayIcon, RefreshIcon, StopIcon, KeyIcon, CalendarCheckIcon, GemIcon // Added CalendarCheckIcon, GemIcon
} from './icons';
...
const GameScreen: React.FC = () => {
  ...
  const generateNewDailyChallenge = useCallback((): DailyChallenge => {
    ...
    return {
      ...
      generatedDate: new Date().toISOString().split('T')[0], // YYYY-MM-DD
      isCompleted: false,
      rewardClaimed: false,
  ...
  const updateDailyChallengeProgress = useCallback((actionType: string, value: number = 1) => {
    setActiveDailyChallenge(prevChallenge => {
      ...
      if (definition.streakChallenge) {
          if (actionType === CHALLENGE_ACTION_CORRECT_ANSWER) { // Assuming value is 1 for correct, 0 for incorrect
              if (value === 1) { // Correct answer
                  newStreak = (newStreak || 0) + 1;
              } else { // Incorrect answer or streak broken by other means
                  newStreak = 0; // Reset streak
              }
              newCurrentValue = Math.max(newCurrentValue, newStreak || 0); // Track max streak achieved
          }
      } else {
          newCurrentValue += value;
      }
      
      newCurrentValue = Math.min(newCurrentValue, prevChallenge.targetValue); // Cap at target
      ...
      if (isNowCompleted && !prevChallenge.isCompleted) {
        playSound(DAILY_CHALLENGE_COMPLETE_SOUND_URL, 0.6);
        showToast(DAILY_CHALLENGE_NEW_AVAILABLE_TEXT, 'info', <CalendarCheckIcon className="w-6 h-6"/>); // Re-using for completion visual
      } else if (newCurrentValue > prevChallenge.currentValue && !isNowCompleted) {
         playSound(DAILY_CHALLENGE_PROGRESS_SOUND_URL, 0.3);
      }
      
      saveActiveDailyChallenge(updatedChallenge);
    ...
    setActiveDailyChallenge(prevChallenge => {
      if (prevChallenge && !prevChallenge.isCompleted) {
        ...
        if (definition?.streakChallenge && navigatingAwayFromIsland) {
          // Only reset streak if navigating away from an island context where streaks matter
          // And if the challenge isn't already about achieving a certain max streak for the day
  ...
  useEffect(() => {
    ...
    if (!challenge || isNewDay(challenge.generatedDate)) {
      ...
      setActiveDailyChallenge(newChallenge);
      saveActiveDailyChallenge(newChallenge);
      playSound(DAILY_CHALLENGE_NEW_SOUND_URL, 0.5);
      showToast(DAILY_CHALLENGE_NEW_AVAILABLE_TEXT, 'info', <CalendarCheckIcon className="w-6 h-6"/>);
    } else {
    ...
    calculateTimeUntilNextRefresh();
    const timerId = setInterval(calculateTimeUntilNextRefresh, 60000); // Update every minute
  ...
  const handleClaimDailyChallengeReward = () => {
    if (activeDailyChallenge && activeDailyChallenge.isCompleted && !activeDailyChallenge.rewardClaimed) {
      setPlayerGems(prevGems => {
        const newGems = prevGems + activeDailyChallenge.rewardGems;
        savePlayerGems(newGems);
        return newGems;
      });
      setActiveDailyChallenge(prev => {
        if (!prev) return null;
        const updated = { ...prev, rewardClaimed: true };
        saveActiveDailyChallenge(updated);
        return updated;
      });
      setCompletedDailyChallengesLog(prev => {
          const newLog = {...prev, [activeDailyChallenge.id]: { date: activeDailyChallenge.generatedDate, challengeId: activeDailyChallenge.id}};
          saveCompletedDailyChallengesLog(newLog);
          return newLog;
      });
      playSound(GEM_COLLECT_SOUND_URL, 0.7);
      showToast(DAILY_CHALLENGE_SUCCESS_TOAST_TEXT(activeDailyChallenge.rewardGems), 'success', <GemIcon className="w-6 h-6"/>);
    }
  };
 ...
 useEffect(() => {
    ...
    if (storedNPC) {
        ...
        if (npc && interaction) {
            setActiveNPCData({ npc, interaction, islandId: storedNPC.islandId, grade: storedNPC.grade });
        } else {
  ...
  const resetForNewIslandPlay = useCallback(() => {
    ...
    // Reset streak challenge progress if it's not a max-streak-for-day type
    setActiveDailyChallenge(prev => {
        if (prev && prev.type === DailyChallengeType.CORRECT_ANSWERS_IN_A_ROW && !prev.isCompleted) {
  ...
  const resetForNewGradeJourney = useCallback((grade: GradeLevel | null) => {
    ...
    resetStreakChallengesIfNeeded(true); // Reset streaks when changing grade
 ...
 const checkAndAwardAchievements = useCallback((completionContext?: { difficulty: IslandDifficulty | null; hintUsed: boolean }) => {
    ...
    if(selectedGrade && islandProgress && Object.keys(islandProgress).length > 0) currentAllGradesProg[selectedGrade] = islandProgress;
    const currentAllGradesStars = {...allGradesStarRatings};
    if(selectedGrade && islandStarRatings && Object.keys(islandStarRatings).length > 0) currentAllGradesStars[selectedGrade] = islandStarRatings;

    const achievementContext: AchievementContext = {
      selectedGrade, islandProgress: progressForCurrentGrade, islandStarRatings: starsForCurrentGrade,
      islandsForGrade: islandsForCurrentGrade, currentOverallScore: overallScore, allGradeIslandConfigs: ISLAND_CONFIGS,
      allGradesProgress: currentAllGradesProg, themeSwapped: themeChangedForAchievement,
      currentIslandDifficulty: completionContext?.difficulty, hintUsedInLastIslandCompletion: completionContext?.hintUsed,
      allGradesStarRatings: currentAllGradesStars, collectedItems, isEndlessUnlockedForGrade, isFinalIslandUnlocked,
      playerGems: playerGems,
      completedDailyChallengesCount: Object.keys(completedDailyChallengesLog).length,
    };
    ALL_ACHIEVEMENTS.forEach(achievement => {
      if (achievedAchievements[achievement.id]) return;
      if (achievement.condition && achievement.condition(achievementContext)) awardAchievement(achievement.id);
    });
  }, [achievedAchievements, selectedGrade, islandProgress, islandStarRatings, islandsForCurrentGrade, overallScore, awardAchievement, allGradesProgress, allGradesStarRatings, themeChangedForAchievement, collectedItems, isEndlessUnlockedForGrade, isFinalIslandUnlocked, playerGems, completedDailyChallengesLog]);


  useEffect(() => {
    const timeoutId = setTimeout(() => checkAndAwardAchievements(), 300);
    return () => clearTimeout(timeoutId);
  }, [islandProgress, islandStarRatings, overallScore, selectedGrade, themeChangedForAchievement, allGradesProgress, allGradesStarRatings, collectedItems, isEndlessUnlockedForGrade, isFinalIslandUnlocked, playerGems, completedDailyChallengesLog, checkAndAwardAchievements]);
      ...
      if (currentIndexInGrade !== -1 && currentIndexInGrade < islandsForCurrentGrade.length - 1) {
        ...
        const timeoutId = setTimeout(() => {
          ...
          if (!cacheEntryForNextIslandSameDifficulty || cacheEntryForNextIslandSameDifficulty === 'pending') {
            backgroundPreloadIslandDifficulty(nextIslandConfig, selectedIslandDifficulty, true).catch(err => console.error(`Error in N+1 preload (same difficulty: ${selectedIslandDifficulty}) for ${nextIslandConfig.name}:`, err));
          }
        }, 2000);
        return () => clearTimeout(timeoutId);
      }
  ...
  const trySpawnTreasureChests = useCallback((grade: GradeLevel) => {
    ...
    setActiveTreasureChests(prevChests => {
        ...
        completedIslandsInGrade.forEach(island => {
            if ( !chestSpawnedThisTime && (!activeMessageBottle || !activeMessageBottle[island.islandId]) && (!activeNPCData || activeNPCData.islandId !== island.islandId || activeNPCData.grade !== grade) && (!activeCollectible || !activeCollectible[island.islandId]) && Math.random() < TREASURE_CHEST_SPAWN_CHANCE ) {
                updatedGradeChests[island.islandId] = true; chestSpawnedThisTime = true; playSound(TREASURE_SPARKLE_SOUND_URL, 0.3);
            }
        });
  ...
  const handleTreasureChestOpened = useCallback((grade: GradeLevel, islandId: string, pointsAwarded: number) => {
    if (pointsAwarded > 0 && selectedGrade) {
        ...
        setOverallScore(newOverallScore); saveOverallScoreToStorage(selectedGrade, newOverallScore);
    ...
    setActiveTreasureChests(prevChests => {
        const updatedGradeChests = { ...(prevChests[grade] || {}) }; delete updatedGradeChests[islandId];
        const newChestsState = { ...prevChests, [grade]: updatedGradeChests }; saveActiveTreasureChestsToStorage(newChestsState); return newChestsState;
    });
  ...
  }, [overallScore, selectedGrade, showToast, updateDailyChallengeProgress, playSound]);
  ...
  const handleMessageBottleOpened = useCallback((islandId: string) => {
    ...
    if (bottleData) {
        ...
        setCurrentBottleMessageContent(messageContent || MESSAGES_IN_BOTTLE[0]); setShowBottleModalForIslandId(islandId); playSound(BOTTLE_OPEN_SOUND_URL, 0.5);
    }
  ...
  }, [shootingStar, selectedGrade, overallScore, playSound, showToast, updateDailyChallengeProgress]);
  ...
  const handleNPCRiddleSubmit = () => {
    ...
    setIsNpcRiddleCorrect(isCorrect); setNpcRiddlePhase('feedback'); let pointsAwarded = 0;
  ...
  const handleNPCModalClose = () => {
    ...
    if (activeNPCData && activeNPCData.interaction.type !== 'riddle' && selectedGrade) {
        ...
        if (pointsAwarded > 0) { const newOverallScore = overallScore + pointsAwarded; setOverallScore(newOverallScore); saveOverallScoreToStorage(selectedGrade, newOverallScore); showToast(`Bạn nhận được ${pointsAwarded} điểm từ ${activeNPCData.npc.name}!`, 'info'); }
    }
  ...
  const handleStartAdventure = () => {
    ...
    if (!process.env.API_KEY) { setApiKeyMissing(true); setGameState('Error'); setLoadingError(API_KEY_ERROR_MESSAGE); return; }
  ...
  const handleGradeSelect = (grade: GradeLevel, isAutoLoading = false) => {
    ...
    if (gradeIslands.length > 0) {
      ...
      setTimeout(() => checkAndAwardAchievements(), 100);
        ...
        if (finalIslandConfig) { setIslandProgress({ [FINAL_TREASURE_ISLAND_ID]: 'unlocked' }); setGameState('IslandMap'); }
        else { setGameState('Error'); setLoadingError("Lỗi: Không tìm thấy cấu hình Đảo Cuối Cùng."); }
    ...
    if (islandConfig && (status === 'unlocked' || status === 'completed')) {
        ...
        if (activeCollectible[islandId]) {
            ...
            if (collectibleInfo) {
                ...
                if (item) { playSound(COLLECTIBLE_COLLECT_SOUND_URL, 0.5); showToast(COLLECTIBLE_COLLECTION_TOAST_MESSAGE(item.name), 'info', <CollectionIcon className="w-6 h-6"/>); setCollectedItems(prev => { const updated = { ...prev, [item.id]: true }; saveCollectedItemsToStorage(updated); return updated; }); }
  ...
  const handleNextQuestionInIsland = useCallback(() => {
    ...
    resetStreakChallengesIfNeeded(false); // Reset streak for current question, if applicable, unless it's the last question

    if (currentQuestionIndexInIsland < questionsForCurrentIsland.length - 1) {
      resetForNewQuestion(); setCurrentQuestionIndexInIsland(prev => prev + 1);
      ...
      if (currentIslandInGradeIndex !== -1 && currentIslandInGradeIndex < islandsForCurrentGrade.length - 1) {
        ...
        if (nextIslandInGrade && updatedProgressForGrade[nextIslandInGrade.islandId] === 'locked') updatedProgressForGrade[nextIslandInGrade.islandId] = 'unlocked';
      }
      setIslandProgress(updatedProgressForGrade); saveIslandProgressToStorage(selectedGrade, updatedProgressForGrade);
      setAllGradesProgress(prev => ({...prev, [selectedGrade]: updatedProgressForGrade}));
      
      // Update Daily Challenge Progress
      updateDailyChallengeProgress(CHALLENGE_ACTION_ISLAND_COMPLETED);
      if (starsEarned > 0) updateDailyChallengeProgress(CHALLENGE_ACTION_STAR_EARNED, starsEarned);
  ...
  const handleAnswerSubmit = useCallback(() => {
    ...
    // Update daily challenge for correct answer streak
  ...
  }, [selectedAnswer, currentQuestion, playerLives, endlessModeLives, gameState, handleNextQuestionInIsland, hintButtonUsed, overallScore, selectedGrade, playSound, unlockAudioContext, updateDailyChallengeProgress, activeDailyChallenge]);

  const handleBackToMap = () => {
    unlockAudioContext(); playSound(BUTTON_CLICK_SOUND_URL);
    resetStreakChallengesIfNeeded(true); // Reset streak when intentionally going back to map
  ...
  const handlePlayIslandAgain = () => {
    ...
    if (currentIslandId && selectedGrade && selectedIslandDifficulty && currentIslandConfig) {
      resetForNewIslandPlay();
      setTransitionDetails({ message: STARTING_ISLAND_TEXT(currentIslandConfig.name, ISLAND_DIFFICULTY_TEXT_MAP[selectedIslandDifficulty]), duration: 700, onComplete: () => setGameState('IslandPlaying') });
      setGameState('Transitioning');
  ...
  const handlePlayThisGradeAgain = () => {
    ...
    if (selectedGrade) {
      ...
      setAllGradesStarRatings(prev => { const updatedAllStars = {...prev, [selectedGrade!]: {}}; saveAllGradesStarRatingsToStorage(updatedAllStars); return updatedAllStars; });
  ...
  const handleChooseAnotherGrade = () => {
    unlockAudioContext(); playSound(BUTTON_CLICK_SOUND_URL); setSelectedGrade(null); saveLastSelectedGrade(null); resetForNewGradeJourney(null); setGameState('GradeSelection');
  };
  ...
  const handleRetryFetchIsland = () => {
    ...
    if (currentIslandId && selectedGrade && selectedIslandDifficulty) {
        ...
        setTransitionDetails(null); setLoadingError(null); fetchAndSetQuestionsForIsland(currentIslandId, selectedIslandDifficulty);
  ...
  const renderStars = (islandId: string) => {
    const stars = islandStarRatings[islandId] || 0; const totalStars = 5;
    return ( <div className="flex justify-center items-center h-6"> {Array.from({ length: totalStars }).map((_, i) => i < stars ? <StarIconFilled key={i} className="w-5 h-5 text-[var(--accent-color)]" /> : <StarIconOutline key={i} className="w-5 h-5 text-[var(--accent-color)] opacity-50" /> )} </div> );
  };
  ...
  const handleAccessFinalIsland = () => {
    if (!isFinalIslandUnlocked) return; playSound(BUTTON_CLICK_SOUND_URL); resetForNewGradeJourney(GradeLevel.FINAL); setSelectedGrade(GradeLevel.FINAL); saveLastSelectedGrade(GradeLevel.FINAL);
    const finalIslandConfig = ISLAND_CONFIGS.find(i => i.islandId === FINAL_TREASURE_ISLAND_ID);
    if (finalIslandConfig) { setIslandProgress({ [FINAL_TREASURE_ISLAND_ID]: 'unlocked'}); setIslandStarRatings({}); setOverallScore(0); setGameState('IslandMap'); }
    else { setGameState('Error'); setLoadingError("Lỗi: Không tìm thấy cấu hình Đảo Kho Báu Cuối Cùng."); }
  };
    ...
    return (
      <div className="w-full animate-fadeInScale flex flex-col items-center justify-center text-center">
        <div className={`p-8 md:p-12 rounded-2xl shadow-2xl bg-[var(--primary-bg)] ${themeConfig.frostedGlassOpacity || ''} max-w-xl`}>
          <h1 className="text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-[var(--title-text-gradient-from)] to-[var(--title-text-gradient-to)] mb-4 md:mb-6"> {GAME_TITLE_TEXT} </h1>
          <p className="text-lg md:text-xl text-[var(--primary-text)] opacity-90 mb-8 md:mb-10"> Chào mừng bạn đến với cuộc phiêu lưu toán học kỳ thú! Sẵn sàng khám phá những hòn đảo bí ẩn và giải các câu đố hóc búa chưa? </p>
          <button onClick={handleStartAdventure} onMouseEnter={() => playSound(HOVER_SOUND_URL, 0.2)} className="bg-gradient-to-r from-[var(--button-primary-bg)] to-[var(--accent-color)] hover:opacity-90 active:brightness-90 text-[var(--button-primary-text)] font-bold py-4 px-10 rounded-xl shadow-xl text-xl md:text-2xl transition-all transform hover:scale-105 active:scale-95"> {START_ADVENTURE_TEXT} </button>
        </div>
      </div>
    );
  }
  ...
  if (gameState === 'GradeSelection') {
    return (
      ...
      <div className="w-full animate-fadeInScale">
        <div className={`w-full max-w-xl mx-auto p-6 md:p-8 bg-[var(--primary-bg)] rounded-2xl shadow-2xl border-2 border-[var(--border-color)] ${themeConfig.frostedGlassOpacity || ''}`}>
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-[var(--title-text-gradient-from)] to-[var(--title-text-gradient-to)] text-center flex-grow"> {CHOOSE_GRADE_TEXT} </h1>
            <div className="flex items-center gap-2">
                 <button onClick={handleToggleDailyChallengeModal} onMouseEnter={() => playSound(HOVER_SOUND_URL, 0.2)} className="p-2.5 rounded-full bg-[var(--button-secondary-bg)] hover:opacity-90 active:brightness-90 shadow-md relative focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-[var(--accent-color)]" aria-label={DAILY_CHALLENGE_BUTTON_TEXT} >
                    <CalendarCheckIcon className="w-6 h-6 sm:w-7 sm:h-7 text-[var(--accent-color)]" />
                    {activeDailyChallenge && !activeDailyChallenge.isCompleted && ( <span className="absolute top-0 right-0 block h-3 w-3 rounded-full bg-red-500 ring-2 ring-white" /> )}
                    {activeDailyChallenge && activeDailyChallenge.isCompleted && !activeDailyChallenge.rewardClaimed && ( <span className="absolute top-0 right-0 block h-3 w-3 rounded-full bg-green-500 ring-2 ring-white animate-pulse" /> )}
                </button>
                <button onClick={handleToggleAchievementsScreen} onMouseEnter={() => playSound(HOVER_SOUND_URL, 0.2)} className="p-2 rounded-full bg-[var(--button-secondary-bg)] hover:opacity-90 active:brightness-90 shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[var(--accent-color)]" aria-label={VIEW_ACHIEVEMENTS_BUTTON_TEXT} > <img src={ACHIEVEMENT_BUTTON_ICON_URL} alt="Huy hiệu" className="w-7 h-7 animate-trophy-glow" /> </button>
            </div>
          </div>
          <div className="flex justify-center items-center mb-4 text-lg font-semibold text-[var(--primary-text)]">
            <GemIcon className="w-6 h-6 mr-2 text-yellow-400" /> {PLAYER_GEMS_TEXT}: {playerGems}
          </div>
          <div className="grid grid-cols-1 gap-4 mb-8">
            {(Object.keys(GradeLevel).filter(key => !isNaN(Number(GradeLevel[key as keyof typeof GradeLevel])) && GradeLevel[key as keyof typeof GradeLevel] !== GradeLevel.FINAL) as (keyof typeof GradeLevel)[]).map((gradeKey) => { const gradeValue = GradeLevel[gradeKey]; return ( <button key={gradeValue} onClick={() => handleGradeSelect(gradeValue as GradeLevel)} onMouseEnter={() => playSound(HOVER_SOUND_URL, 0.2)} className="p-4 rounded-lg shadow-lg transition-transform transform hover:scale-105 active:scale-95 active:brightness-90 bg-[var(--button-primary-bg)] hover:opacity-90 text-[var(--button-primary-text)] font-bold text-2xl"> {GRADE_LEVEL_TEXT_MAP[gradeValue as GradeLevel]} </button> ); })}
             {isFinalIslandUnlocked && ( <button onClick={handleAccessFinalIsland} onMouseEnter={() => playSound(HOVER_SOUND_URL, 0.2)} className="mt-4 p-4 rounded-lg shadow-lg transition-transform transform hover:scale-105 active:scale-95 active:brightness-90 bg-gradient-to-r from-yellow-400 via-amber-500 to-orange-500 text-white font-bold text-2xl flex items-center justify-center gap-2 animate-pulse-glow" style={{ ['--island-button-ring-color' as any]: 'gold' }}> <KeyIcon className="w-8 h-8" /> {FINAL_ISLAND_ACCESS_BUTTON_TEXT} </button> )}
          </div>
      ...
      {showDailyChallengeModal && activeDailyChallenge && ( <DailyChallengeModal isOpen={showDailyChallengeModal} challenge={activeDailyChallenge} onClose={handleToggleDailyChallengeModal} onClaimReward={handleClaimDailyChallengeReward} playSound={playSound} timeUntilNextRefresh={timeUntilNextDailyChallengeRefresh} /> )}
  ...
  if (showBottleModalForIslandId && currentBottleMessageContent) {
    ...
    return ( <> <MessageInBottleModal isOpen={true} islandName={islandConfigForBottle?.name || "Một hòn đảo"} message={currentBottleMessageContent} onClose={handleMessageBottleClosed} playSound={playSound} themeConfig={themeConfig} /> {showAchievementsScreen && ( <AchievementsScreen achievedAchievements={achievedAchievements} onClose={handleToggleAchievementsScreen} playSound={playSound} currentGradeContext={selectedGrade} collectedItems={collectedItems} allCollectibles={COLLECTIBLE_ITEMS} /> )} {showDailyChallengeModal && activeDailyChallenge && ( <DailyChallengeModal isOpen={showDailyChallengeModal} challenge={activeDailyChallenge} onClose={handleToggleDailyChallengeModal} onClaimReward={handleClaimDailyChallengeReward} playSound={playSound} timeUntilNextRefresh={timeUntilNextDailyChallengeRefresh} /> )} <ToastNotification toast={currentToast} onDismiss={() => setCurrentToast(null)} /> </> );
  }

  if (showNPCModal && activeNPCData) {
    const islandConfigForNPC = ISLAND_CONFIGS.find(i => i.islandId === activeNPCData.islandId);
    return ( <> <FriendlyNPCModal isOpen={true} npcData={activeNPCData.npc} interactionContent={activeNPCData.interaction} islandName={islandConfigForNPC?.name || "Nơi bí ẩn"} onClose={handleNPCModalClose} playSound={playSound} themeConfig={themeConfig} onSubmitRiddle={handleNPCRiddleSubmit} riddleAnswerInput={npcRiddleAnswer} onRiddleAnswerChange={setNpcRiddleAnswer} riddlePhase={npcRiddlePhase} isRiddleCorrect={isNpcRiddleCorrect} /> {showAchievementsScreen && ( <AchievementsScreen achievedAchievements={achievedAchievements} onClose={handleToggleAchievementsScreen} playSound={playSound} currentGradeContext={selectedGrade} collectedItems={collectedItems} allCollectibles={COLLECTIBLE_ITEMS} /> )} {showDailyChallengeModal && activeDailyChallenge && ( <DailyChallengeModal isOpen={showDailyChallengeModal} challenge={activeDailyChallenge} onClose={handleToggleDailyChallengeModal} onClaimReward={handleClaimDailyChallengeReward} playSound={playSound} timeUntilNextRefresh={timeUntilNextDailyChallengeRefresh} /> )} <ToastNotification toast={currentToast} onDismiss={() => setCurrentToast(null)} /> </> );
  }


  if (gameState === 'IslandMap' && selectedGrade) {
    if (islandsForCurrentGrade.length === 0 && selectedGrade !== GradeLevel.FINAL) {
        return ( <> <div className="w-full animate-fadeInScale"> <div className={`p-8 rounded-lg shadow-xl text-center max-w-lg mx-auto bg-[var(--secondary-bg)] text-[var(--secondary-text)] ${themeConfig.frostedGlassOpacity || ''}`}> <AlertTriangleIcon className="w-16 h-16 mx-auto mb-4 text-[var(--accent-color)]" /> <h1 className="text-3xl font-bold mb-4">Thông Báo Cấu Hình</h1> <p className="text-xl mb-6"> {NO_ISLANDS_FOR_GRADE_TEXT} </p> <button onClick={handleChooseAnotherGrade} className="bg-[var(--button-secondary-bg)] hover:opacity-90 active:brightness-90 text-[var(--button-secondary-text)] font-bold py-3 px-6 rounded-lg"> {CHOOSE_ANOTHER_GRADE_TEXT} </button> </div> </div> {showAchievementsScreen && ( <AchievementsScreen achievedAchievements={achievedAchievements} onClose={handleToggleAchievementsScreen} playSound={playSound} currentGradeContext={selectedGrade} collectedItems={collectedItems} allCollectibles={COLLECTIBLE_ITEMS} /> )} {showDailyChallengeModal && activeDailyChallenge && ( <DailyChallengeModal isOpen={showDailyChallengeModal} challenge={activeDailyChallenge} onClose={handleToggleDailyChallengeModal} onClaimReward={handleClaimDailyChallengeReward} playSound={playSound} timeUntilNextRefresh={timeUntilNextDailyChallengeRefresh} /> )} <ToastNotification toast={currentToast} onDismiss={() => setCurrentToast(null)} /> </> );
    }
    ...
    return (
      ...
      <div className="w-full animate-fadeInScale relative">
        {shootingStar && shootingStar.visible && !shootingStar.clicked && ( <ShootingStar starData={shootingStar} onClick={() => handleShootingStarClick(shootingStar.id)} onDisappear={() => setShootingStar(prev => prev && prev.id === shootingStar.id ? null : prev)} emoji={SHOOTING_STAR_EMOJI} /> )}
        <div className={`w-full max-w-4xl mx-auto p-6 md:p-8 bg-[var(--primary-bg)] rounded-2xl shadow-2xl border-2 border-[var(--border-color)] ${themeConfig.frostedGlassOpacity || ''}`}>
          <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-2">
              <h1 className="text-3xl md:text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-[var(--title-text-gradient-from)] to-[var(--title-text-gradient-to)] mb-2 sm:mb-0"> {selectedGrade === GradeLevel.FINAL ? FINAL_ISLAND_GRADE_TITLE : CHOOSE_ISLAND_TEXT} </h1>
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="flex items-center gap-1.5 sm:gap-2 p-1.5 sm:p-2 rounded-full bg-[var(--secondary-bg)] shadow-md">
                    <GemIcon className="w-5 h-5 sm:w-6 sm:h-6 text-yellow-400" />
                    <span className="text-sm sm:text-base font-semibold text-[var(--primary-text)]">{playerGems}</span>
                </div>
                 <button onClick={handleToggleDailyChallengeModal} onMouseEnter={() => playSound(HOVER_SOUND_URL, 0.2)} className="p-2.5 rounded-full bg-[var(--button-secondary-bg)] hover:opacity-90 active:brightness-90 shadow-md relative focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-[var(--accent-color)]" aria-label={DAILY_CHALLENGE_BUTTON_TEXT} >
                    <CalendarCheckIcon className="w-6 h-6 sm:w-7 sm:h-7 text-[var(--accent-color)]" />
                     {activeDailyChallenge && !activeDailyChallenge.isCompleted && ( <span className="absolute top-0 right-0 block h-3 w-3 rounded-full bg-red-500 ring-2 ring-white" /> )}
                ...
                <button onClick={handleChooseAnotherGrade} onMouseEnter={() => playSound(HOVER_SOUND_URL, 0.2)} className="bg-[var(--button-secondary-bg)] hover:opacity-90 active:brightness-90 text-[var(--button-secondary-text)] font-semibold py-2 px-3 sm:px-4 rounded-lg shadow-md text-xs sm:text-sm"> {CHOOSE_ANOTHER_GRADE_TEXT} </button>
                <button onClick={handleToggleAchievementsScreen} onMouseEnter={() => playSound(HOVER_SOUND_URL, 0.2)} className="hover:opacity-90 active:brightness-90 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-[var(--accent-color)] rounded-full" aria-label={VIEW_ACHIEVEMENTS_BUTTON_TEXT} > <img src={ACHIEVEMENT_BUTTON_ICON_URL} alt="Huy hiệu" className="w-12 h-12 sm:w-14 sm:h-14 animate-trophy-glow" /> </button>
              </div>
          </div>
          ...
          <p className="text-center text-[var(--primary-text)] opacity-90 mb-1 text-2xl"> {selectedGrade === GradeLevel.FINAL ? islandsForCurrentGrade[0]?.name : `Lớp: ${GRADE_LEVEL_TEXT_MAP[selectedGrade]}`} </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {islandsForCurrentGrade.map((island) => {
              const status = islandProgress[island.islandId] || 'locked'; const isDisabled = status === 'locked'; let currentBgColor = 'bg-[var(--island-locked-bg)]'; let currentTextColor = 'text-[var(--island-locked-text)]';
              ...
              const isUnlockedAndNotCompleted = status === 'unlocked'; const hasTreasure = status === 'completed' && selectedGrade && activeTreasureChests[selectedGrade]?.[island.islandId]; const hasBottle = status === 'completed' && activeMessageBottle[island.islandId]; const hasNPC = (status === 'completed' || status === 'unlocked') && activeNPCData && activeNPCData.islandId === island.islandId && activeNPCData.grade === selectedGrade; const collectibleOnIsland = status === 'completed' && activeCollectible[island.islandId]; const collectibleIcon = collectibleOnIsland ? COLLECTIBLE_ITEMS.find(c => c.id === activeCollectible[island.islandId]?.collectibleId)?.icon : null;
              const pulseAnimation = isUnlockedAndNotCompleted || hasTreasure || hasBottle || hasNPC || collectibleOnIsland; let topIcon = null;
              if (collectibleOnIsland && collectibleIcon) topIcon = collectibleIcon; else if (hasNPC && activeNPCData?.islandId === island.islandId) topIcon = <img src={activeNPCData.npc.imageUrl} alt={activeNPCData.npc.name} className="w-full h-full object-contain rounded-full" />; else if (hasTreasure) topIcon = TREASURE_CHEST_ICON_EMOJI; else if (hasBottle) topIcon = MESSAGE_IN_BOTTLE_ICON_EMOJI;
              ...
              return ( <button key={island.islandId} onClick={() => !isDisabled && handleIslandSelect(island.islandId)} onMouseEnter={() => !isDisabled && playSound(HOVER_SOUND_URL, 0.2)} disabled={isDisabled} className={`p-4 rounded-lg shadow-lg transition-all duration-200 transform hover:scale-105 ${currentBgColor} ${currentTextColor} min-h-[180px] flex flex-col justify-between items-center text-center focus:outline-none focus:ring-4 ring-[var(--island-button-ring-color)] relative ${isDisabled ? 'opacity-70 cursor-not-allowed' : 'hover:opacity-90 active:scale-95 active:brightness-90'} ${pulseAnimation ? 'animate-pulse-glow' : ''} ${(hasTreasure || hasBottle || hasNPC || collectibleOnIsland) ? 'ring-4 ring-yellow-400 border-2 border-yellow-200' : ''} `} aria-label={ariaLabelParts.join(' ')} > {topIcon && ( <span className="absolute top-1 right-1 text-2xl animate-bounce" style={{ filter: 'drop-shadow(0 0 3px gold)', width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center' }} > {topIcon} </span> )} <div className="flex-grow flex flex-col items-center justify-center"> <span className="text-3xl mb-1" aria-hidden="true">{island.mapIcon}</span> <h2 className="text-lg font-bold leading-tight">{island.name}</h2> <p className="text-xs mt-1 px-1 opacity-90">{island.description}</p> </div> <div className="mt-2 h-6"> {isDisabled && <LockIcon className="w-6 h-6 text-[var(--incorrect-bg)] opacity-70" />} {status === 'completed' && !topIcon ? <div className="animate-subtle-shine">{renderStars(island.islandId)}</div> : null} {((status === 'completed' || status === 'unlocked') && topIcon ) ? <div className="animate-subtle-shine">{renderStars(island.islandId)}</div> : null} {status === 'unlocked' && !topIcon ? <span className="text-xs opacity-80">(Chưa hoàn thành)</span> : null} </div> </button> ); })}
          </div>
          {selectedGrade !== GradeLevel.FINAL && ( <p className="text-center text-[var(--primary-text)] opacity-90 mt-8 text-2xl font-bold">Tổng Điểm {GRADE_LEVEL_TEXT_MAP[selectedGrade]}: {overallScore}</p> )}
           {selectedGrade !== GradeLevel.FINAL && isEndlessUnlockedForGrade[selectedGrade] && ( <button onMouseEnter={() => playSound(HOVER_SOUND_URL, 0.2)} className="mt-6 w-full sm:w-auto mx-auto block bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-600 hover:to-indigo-700 text-white font-bold py-3 px-8 rounded-lg shadow-lg text-lg transition-transform transform hover:scale-105 flex items-center justify-center gap-2"> <PlayIcon className="w-6 h-6" /> {ENDLESS_MODE_BUTTON_TEXT} </button> )}
        </div>
      </div>
      {showAchievementsScreen && ( <AchievementsScreen achievedAchievements={achievedAchievements} onClose={handleToggleAchievementsScreen} playSound={playSound} currentGradeContext={selectedGrade} collectedItems={collectedItems} allCollectibles={COLLECTIBLE_ITEMS} /> )}
      {showDailyChallengeModal && activeDailyChallenge && ( <DailyChallengeModal isOpen={showDailyChallengeModal} challenge={activeDailyChallenge} onClose={handleToggleDailyChallengeModal} onClaimReward={handleClaimDailyChallengeReward} playSound={playSound} timeUntilNextRefresh={timeUntilNextDailyChallengeRefresh}/> )}
  ...
  if (gameState === 'IslandComplete' && currentIslandConfig && selectedGrade && selectedIslandDifficulty && currentIslandId) {
    ...
    const nextIsland = (currentCompletedIslandIndex !== -1 && currentCompletedIslandIndex < islandsForCurrentGrade.length - 1) ? islandsForCurrentGrade[currentCompletedIslandIndex + 1] : null;
    const canGoToNextIsland = nextIsland && islandProgress[nextIsland.islandId] === 'unlocked';
    const starsAchievedForThisIsland = islandStarRatings[currentIslandId] || 0; const isPerfectRun = starsAchievedForThisIsland === 5;
    let specialCelebrationText = ""; let showBigBlinkingStar = false; let perfectRunMessage = `Bạn đạt được: ${islandScore} điểm cho đảo này.`;
    if (isPerfectRun) { if (selectedIslandDifficulty === IslandDifficulty.HARD) { specialCelebrationText = REWARD_TEXT_HARD_PERFECT; showBigBlinkingStar = true; } else if (selectedIslandDifficulty === IslandDifficulty.MEDIUM) { specialCelebrationText = REWARD_TEXT_MEDIUM_PERFECT; showBigBlinkingStar = true; } else { perfectRunMessage = REWARD_TEXT_EASY_PERFECT; } }
