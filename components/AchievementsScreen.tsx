...
import { Achievement, AchievedAchievementsState, GradeLevel } from '../types';
...
const AchievementsScreen: React.FC<AchievementsScreenProps> = ({ 
  ...
  const filteredAchievements = useMemo(() => {
    ...
    return achievementsToDisplay.sort((a, b) => {
      ...
      if (!achievedA && achievedB) return 1;
      // Optional: alphabetical sort for unachieved items
      if (!achievedA && !achievedB) return a.name.localeCompare(b.name);
      // Optional: sort achieved by date (most recent first) or name
  ...
  const progressPercentage = totalAchievements > 0 ? (achievedCount / totalAchievements) * 100 : 0;
      ...
      >
        ...
        <header className="mb-3 sm:mb-4 text-center">
          ...
          <h2 id="achievements-modal-title" className="text-xl sm:text-2xl font-bold text-[var(--modal-header-text)]">
           ...
           <div className="mt-1.5 sm:mt-2 text-xs sm:text-sm text-[var(--secondary-text)] opacity-90 px-2">
            <p>Đã đạt: {achievedCount} / {totalAchievements} huy hiệu ({progressPercentage.toFixed(0)}%)</p>
