...
import {
    ...
    ThemeAccessory, PlayerOwnedAccessoriesState // Added Shop related types
} from '../types';
import {
    ...
    SHOP_ACCESSORIES, PLAYER_OWNED_ACCESSORIES_KEY, SHOP_TITLE_TEXT, SHOP_BACK_BUTTON_TEXT // Added Shop constants
} from '../constants';
...
import ShopScreen from './ShopScreen'; // Added ShopScreen import
...
import {
    ...
    ShoppingBagIcon // Added ShoppingBagIcon
} from './icons';
...
const savePlayerGems = (gems: PlayerGemsState) => {
  ...
  dispatchGemsUpdate(gems); // Dispatch event when gems are saved
};
...
// Shop related storage functions
...
const GameScreen: React.FC = () => {
  ...
  // New state for Final Island sequential loading
 ...
 useEffect(() => {
    ...
    // Load owned accessories on mount
 ...
 const checkAndAwardAchievements = useCallback((completionContext?: { difficulty: IslandDifficulty | null; hintUsed: boolean }) => {
    ...
    if (isCorrect) {
        ...
        switch (gameState) {
            case 'EndlessPlaying':
                ...
                break;
            case 'IslandPlaying':
                // selectedGrade is guaranteed non-null here due to the initial guard
                if (selectedGrade) { // Added for type safety, though initial guard should cover
      ...
      setSelectedAnswer, setUserAttemptShown, setPlayerLives // Added missing state setters
    ]);
  ...
  const handleBuyAccessory = (accessory: ThemeAccessory) => {
    if (playerGems >= accessory.price && !playerOwnedAccessories[accessory.id]) {
        ...
        playSound(GEM_COLLECT_SOUND_URL, 0.6); // Or a specific "buy" sound
